...
The SDK requires Node.js 22.13 or later. It ships per-platform `@cursor/sdk- - ` binaries for sandboxing and ripgrep, so it is a Node-first package.
...
`agent.agentId` is populated immediately. Local agents get an `agent- ` ID; cloud agents get a `bc- ` ID.
...
| Member | Description |
| :---------------------- | :--------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `agentId` | Stable agent identifier. `agent- ` for local, `bc- ` for cloud. |
...
| `onStep` | `(args: { step }) => void \| Promise ` | Callback after each completed conversation step (text, thinking, or tool batch). |
| `onDelta` | `(args: { update }) => void \| Promise ` | Callback per raw `InteractionUpdate`. |
...
- **Shell** — Commands run inside a platform sandbox (`bubblewrap` on Linux, `seatbelt` on macOS, the bundled `@cursor/sdk- - ` helper). Privileged operations are denied.
...
Config for local agents, passed as `local` on `Agent.create()`. Also exported as a standalone type for `Partial `.
...
| `mcpServers` | `Array >` | | MCP servers available to this subagent. Names reference servers from the parent's `mcpServers`. |
