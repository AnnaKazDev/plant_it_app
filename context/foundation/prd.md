...
- Weather data displayed for historical actions reflects conditions at the time the action was recorded. If weather data cannot be retrieved during action creation, the application shows a clear error message and allows the user to retry or save the action without weather data.
...
(No open questions — all required content is present in the input)
