...
-- Run with: npx supabase db query -f scripts/create-test-data.sql

-- First, get or create a user (you need to sign up via UI first)
...
BEGIN
  -- Get first user (or specific user email)
  ...
  RAISE NOTICE '';
  RAISE NOTICE 'Use this action_id for curl test: %', v_action_id;
END $$;
